...
from bot.modules.func import check_filename
...
class YoutubeDLHelper:
    ...
    async def add_download(self, path, qual, playlist, options):
        ...
        msg = await check_filename(message=self._listener.message, file_name=self._listener.name, link=self._listener.link)
        if msg:
