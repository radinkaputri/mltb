...
from bot.modules.func import copy_message, edit_video_metadata, update_leech_links, add_attachment
...
class TgUploader:
    ...
    async def _msg_to_reply(self):
        ...
        if self._listener.upDest:
            ...
            msg += f"\n<b>┖<a href='{self._listener.message.link if self._listener.isSuperChat else self._listener.message.text.lstrip('/')}'>Leech Source Link</a></b></blockquote>"
            try:
                if self._user_session:
    ...
    async def _upload_file(self, cap_mono, file, o_path, force_document=False):
        ...
        try:
            ...
            if config_dict['LEECH_DUMP_CHAT'] and config_dict['BOT_PM']:
                await copy_message(chat_id=self._listener.message.from_user.id, from_chat_id=self._sent_msg.chat.id, message_id=self._sent_msg.id)
            await update_leech_links(name=self._listener.name, from_chat_id=self._sent_msg.chat.id, message_id=self._sent_msg.id)
