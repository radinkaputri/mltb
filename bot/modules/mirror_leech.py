...
from bot.modules.func import delete_links, command_listener, get_tag, check_filename, send_to_chat, task_utils
...
class Mirror(TaskListener):
    ...
    async def newEvent(self):
        ...
        tag = await get_tag(self.message)
        ...
        if reply_to:
            ...
            error_msg = []
            error_button = None
            task_utilis_msg, error_button = await task_utils(message=self.message)
            if task_utilis_msg:
            ...
            if error_msg:
                ...
                await send_to_chat(message=self.message, text=final_msg, buttons=error_button, reply=True, photo=False)
                return
            
            if file_ is not None:
                msg = await check_filename(message=self.message, file_name=file_.file_name, link=self.link)
                if msg:
                    await sendMessage(self.message, text=f"{tag} {msg}")
                    return
...
async def jd_mirror(client, message):
    if await command_listener(message, isJd=True):
        return
    Mirror(client, message, isJd=True).newEvent()


async def jd_leech(client, message):
    if await command_listener(message, isJd=True, isLeech=True):
        return
